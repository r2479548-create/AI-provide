...
test("foreign OAuth session softly redirects cache affinity while the same session stays local", () => {
  ...
  const fixture = Array.from({ length: 10_000 }, (_, index) => {
    ...
    const baseline = applyPromptCacheAffinity(oauthTargets, body, true, "session-a").targets;
    const occupied = baseline[0];
    const alternative = baseline[1];
    const release = reserveOAuthSession(occupied.connectionId!, "session-a");
    const foreignFirst = applyPromptCacheAffinity(oauthTargets, body, true, "session-b").targets[0];
    release();
  ...
  assert.equal(
    applyPromptCacheAffinity(oauthTargets, body, true, "session-a").targets[0].connectionId,
    occupied.connectionId,
    "the owning session keeps its cache-local account"
  );
  assert.equal(
    applyPromptCacheAffinity(oauthTargets, body, true, "session-b").targets[0].connectionId,
