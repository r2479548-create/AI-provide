...
test("/api/v1/images allows a 20 MB base64 payload (file under 10 MB after inflation)", () => {
  ...
  assert.equal(
    checkBodySize(request, getBodySizeLimit("/api/v1/images/generations", { maxBodySizeMb: 10 })),
    null
  );
});
