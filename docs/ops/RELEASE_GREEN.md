...
`check:node-runtime`, and `npm run build`. Trusted release runners use Turbopack; fork-safe
GitHub-hosted runners use the documented webpack fallback because Turbopack's native memory is
not bounded by `NODE_OPTIONS`. The job intentionally does not upload a build artifact because no
downstream quality job consumes one in this workflow.
